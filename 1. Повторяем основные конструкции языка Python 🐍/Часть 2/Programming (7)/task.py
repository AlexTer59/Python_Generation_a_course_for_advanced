...
#
# Формат входных данных На вход программе подаются две строки текста, содержащие по одному слову из перечня "камень",
...
vars = ['камень', 'ящерица', 'Спок', 'ножницы', 'бумага']


def who_wins(pl1, pl2):
    if pl1 == pl2:
        return "ничья"
    if (vars.index(pl1) - vars.index(pl2)) % 2 == 0:
        if vars.index(pl1) > vars.index(pl2):
            return "Тимур"
        else:
            return "Руслан"
    elif (vars.index(pl1) - vars.index(pl2)) % 2 != 0:
        if vars.index(pl1) > vars.index(pl2):
            return "Руслан"
        else:
            return "Тимур"
