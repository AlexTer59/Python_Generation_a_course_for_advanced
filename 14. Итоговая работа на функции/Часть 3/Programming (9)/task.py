...
#
# Примечание 3. Пусть требуется вычислить гематрию слова BaSis. Переводим его в верхний регистр BASIS. Для каждого
# символа в слове находим его код с помощью встроенной функции ord():
#
# ord('B') = 66
# ord('A') = 65
# ord('S') = 83
...
# ord('S') - ord('A') = 83 - 65 = 18
#
# Гем атрия слова BaSis равна 1+0+18+8+18=45.
...
def gemary(lst):
    return reduce(lambda acc, x: acc + (ord(x.upper()) - ord('A')), list(lst), 0)


lst = [input() for _ in range(int(input()))]
print(*sorted(lst, key=lambda x: (gemary(x), x)), sep='\n')
